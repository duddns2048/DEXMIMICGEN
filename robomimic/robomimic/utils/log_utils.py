...
class DataLogger(object):
        ...
        if data_type == 'scalar':
            ...
            if log_stats or k in self._data: # any key that we're logging or previously logged
                if k not in self._data:
                    self._data[k] = dict()
                assert epoch not in self._data[k], "tried to record multiple values for same epoch {} under key {}".format(epoch, k)
                self._data[k][epoch] = v
    ...
    def get_stats(self, k):
        ...
        """
        stats = dict()
        values = list(self._data[k].values()) # data has dictionary mapping epoch to values
